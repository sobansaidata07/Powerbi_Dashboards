-- ============================================================================
-- TRAI Telecom Analytics 2025-26 — Google BigQuery Analysis
-- 12 analytical queries answering 12 business questions on the telecom dataset
-- Project ID anonymized — replace `your_project` with your own GCP project ID
-- ============================================================================


-- ============================================================================
-- Query 1: Competitive Subscriber Race
-- ============================================================================
WITH base AS (
  SELECT 'Jio' AS Operator, ROUND(SUM(Net_Add_Jio_Mn), 2) AS Growth_Millions
  FROM `your_project.Telecom_Industry_Dataset.OPERATOR_PERFORMANCE`
  UNION ALL
...
SELECT
  DENSE_RANK() OVER (ORDER BY base.Growth_Millions DESC) AS rnk,
  *
FROM base;
...
WITH base AS (
  ...
  FROM `your_project.Telecom_Industry_Dataset.FWA_AND_5G_BROADBAND` AS a
  JOIN `your_project.Telecom_Industry_Dataset.REPORTING_PERIOD` AS b
    ON b.Month = a.Month
)
SELECT
  Month, Current_month,
...
WITH tele_density AS (
  SELECT
    ...
    ROUND(a.Rural_Tele_Density_Pct - LAG(a.Rural_Tele_Density_Pct) OVER (ORDER BY b.Period_end), 2) AS rural_growth
  FROM `your_project.Telecom_Industry_Dataset.TELE_DENSITY_DIGITAL_INCLUSION` a
  JOIN `your_project.Telecom_Industry_Dataset.Trend_analysis` b
    ON a.Month = b.Month
)
SELECT
  Month, Urban_Tele_Density_Pct, Rural_Tele_Density_Pct, density_gap, rural_growth,
  DENSE_RANK() OVER (ORDER BY rural_growth DESC) AS Rural_rnk
FROM tele_density
ORDER BY Period_end;
...
SELECT
  ...
  a.MNP_Zone1_Mn,
  a.MNP_Zone2_Mn
FROM `your_project.Telecom_Industry_Dataset.MNP_AND_CHURN` AS a
JOIN `your_project.Telecom_Industry_Dataset.Trend_analysis` AS b
  ON a.Month = b.Month
...
SELECT
  a.Month,
  a.Active_VLR_Subs_Mn,
  a.VLR_Pct_of_Total
FROM `your_project.Telecom_Industry_Dataset.ACTIVE_SUBSCRIBERS` AS a
JOIN `your_project.Telecom_Industry_Dataset.Trend_analysis` AS b
  ON a.Month = b.Month
ORDER BY b.Period_End ASC;
...
SELECT
  ...
  ROUND((a.Total_Wireline_Subs_Mn - LAG(a.Total_Wireline_Subs_Mn) OVER (ORDER BY b.Period_End ASC)), 2) AS Diff_wireline_subs,
  ROUND((a.Total_Wireline_Subs_Mn - LAG(a.Total_Wireline_Subs_Mn) OVER (ORDER BY b.Period_End ASC)) * 100 /
        LAG(a.Total_Wireline_Subs_Mn) OVER (ORDER BY b.Period_End ASC), 2) AS Diff_wireline_subs_in_percent,
  a.Total_Telephone_Subs_Mn
FROM `your_project.Telecom_Industry_Dataset.SUBSCRIBER_BASE_MARKET_SIZE` AS a
JOIN `your_project.Telecom_Industry_Dataset.Trend_analysis` AS b
  ON a.Month = b.Month
ORDER BY b.Period_End ASC;
...
WITH base AS (
  SELECT
    ...
    ROUND(((a.M2M_Connections_Mn - LAG(a.M2M_Connections_Mn) OVER (ORDER BY b.Period_End)) * 100) /
          LAG(a.M2M_Connections_Mn) OVER (ORDER BY b.Period_End), 2) AS avg_monthly_growth
  FROM `your_project.Telecom_Industry_Dataset.IOT_ENTERPRISE_CONNECTIVITY` AS a
  JOIN `your_project.Telecom_Industry_Dataset.Trend_analysis` AS b
    ON a.Month = b.Month
  ORDER BY b.Period_End ASC
)
...
FROM `your_project.Telecom_Industry_Dataset.MARKET_SHARE_COMPETITION` AS a
JOIN `your_project.Telecom_Industry_Dataset.Trend_analysis` AS b
  ON a.Month = b.Month
...
WITH base AS (
  ...
  FROM `your_project.Telecom_Industry_Dataset.BROADBAND_INTERNET_ADOPTION` AS a
  JOIN `your_project.Telecom_Industry_Dataset.Trend_analysis` AS b
    ON a.Month = b.Month
  ORDER BY b.Period_End
)
SELECT
  ...
  ROUND(
    ROUND((Fixed_Wireless_Broadband_Mn * 100 / Total_Broadband_Subs_Mn), 2) +
    ROUND((Fixed_Wired_Broadband_Mn * 100 / Total_Broadband_Subs_Mn), 2), 2
  ) AS Fixed
FROM base;
...
WITH base AS (
  SELECT
    ...
    ROUND((a.Net_Add_Total_Mn - LAG(a.Net_Add_Total_Mn) OVER (ORDER BY b.Period_End)), 2) AS difference_Mn
  FROM `your_project.Telecom_Industry_Dataset.SUBSCRIBER_GROWTH` AS a
  JOIN `your_project.Telecom_Industry_Dataset.Trend_analysis` AS b
    ON a.Month = b.Month
  ORDER BY b.Period_End
)
SELECT
  (MAX(Net_Add_Total_Mn) - MIN(Net_Add_Total_Mn)) AS diff,
  ROUND(STDDEV(Net_Add_Total_Mn)) AS stdd
FROM base;
...
FROM `your_project.Telecom_Industry_Dataset.OPERATOR_PERFORMANCE` AS a
JOIN `your_project.Telecom_Industry_Dataset.Trend_analysis` AS b
  ON a.Month = b.Month
ORDER BY b.Period_End;
...
-- ============================================================================
SELECT
  a.Month, a.`5G_FWA_Subs_Mn`, a.Urban_5G_FWA_Subs_Mn, a.Rural_5G_FWA_Subs_Mn,
